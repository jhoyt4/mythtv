...
# Per the Apple developer notes, you must codesign from the inside out meaning
# libraries first, then frameworks, and finally the applications.
for DIRECTORY in Resources PlugIns Frameworks; do
  ...
  for file in $APP/Contents/$DIRECTORY/**/*(.); do 
  	case $file in
        *.dylib|*.so|Qt*|Python)
           /usr/bin/codesign --force -s "${CODESIGN_ID}" -v --timestamp -o runtime --entitlements $ENTITLEMENT --continue "$file"
        ;;
    esac
  done
done
...
/usr/bin/codesign --force -s "${CODESIGN_ID}" -v --timestamp -o runtime --entitlements $ENTITLEMENT --continue $APP/Contents/Frameworks/*.framework

# Finally sign the application
echo "Codesigning Application"
/usr/bin/codesign --force -s "${CODESIGN_ID}" -v --timestamp -o runtime --entitlements $ENTITLEMENT --continue $APP
